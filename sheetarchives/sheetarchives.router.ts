...
import { Sheetarchive } from './sheetarchives.model'
...
class SheetarchivesRouter extends ModelRouter<Sheetarchive> {
    constructor() {
        super(Sheetarchive)
    }

    protected prepareOne(query: mongoose.DocumentQuery<Sheetarchive, Sheetarchive>): mongoose.DocumentQuery<Sheetarchive, Sheetarchive> {
        return query
    ...
    envelop(document) {
        ...
        const sheetarchiveID = document.sheetarchive._id ? document.sheetarchive._id : document.sheetarchive
        resource._links.departament = `/sheetarchives/${sheetarchiveID}`
        return resource
    }
    ...
    save = async (req, resp, next) => {
        ...
        // let sheetName = await Sheetarchive.find({ sheet: req.body.sheet })

        // if (sheetName.length === 1) {
        //     await Sheetarchive.updateOne({ sheet: req.body.sheet, mailSignup: req.authenticated.mailSignup }, {
        //         $push: {
        //             logErrors:
        //             {
        //                 $each: [
    ...
    excel = async (req, resp, next) => {
        ...
        try {
            ...
            let result = await Sheetarchive.find({ _id: req.params.id })
            ...
            let _tag = await log.map(el => { return el.tag });
            let tag = [].concat.apply([], _tag) // retorna a localizaçã
            ...
            ws.cell(1, 3).string('INDICES').style(style2)
            ws.cell(1, 4).string('TIPO ERRO').style(style2) 

            for (let i = 0; row.length > i && location.length > i && msgError.length > i && tag.length > i; i++) {
                ws.cell(i + 2, 1).number(row[i]).style(style1)
                ws.cell(i + 2, 2).string(location[i]).style(style1)
                ws.cell(i + 2, 3).string(tag[i]).style(style1)
                ws.cell(i + 2, 4).string(msgError[i]).style(style1)
            } 
...
export const sheetarchivesRouter = new SheetarchivesRouter()
